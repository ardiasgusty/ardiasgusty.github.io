...
const CAR_DATA = {
  brio:{name:'Honda Brio RS',maxSpeed:44,accel:23,handling:2.65,width:1.68,length:3.82,height:1.49,color:0xe81d2d,type:'hatch'},
  wrv:{name:'Honda WR-V RS',maxSpeed:46,accel:21,handling:2.35,width:1.78,length:4.06,height:1.61,color:0xf4f5f7,type:'suv'},
  hrv:{name:'Honda HR-V e:HEV',maxSpeed:48,accel:22,handling:2.28,width:1.79,length:4.35,height:1.59,color:0x33485f,type:'suvCoupe'},
  civic:{name:'Honda Civic RS',maxSpeed:53,accel:24,handling:2.48,width:1.80,length:4.68,height:1.42,color:0xd1d4d8,type:'sedan'},
  typer:{name:'Honda Civic Type R',maxSpeed:58,accel:27,handling:2.72,width:1.89,length:4.59,height:1.40,color:0xffffff,type:'typeR'}
};
...
function createCarModel(data, colorOverride){
  const color=colorOverride ?? data.color;
  const group=new THREE.Group();
  const bodyMat=new THREE.MeshPhysicalMaterial({color,metalness:.18,roughness:.32,clearcoat:.75,clearcoatRoughness:.18});
  const dark=new THREE.MeshStandardMaterial({color:0x11151a,roughness:.35,metalness:.1});
  const glass=new THREE.MeshPhysicalMaterial({color:0x23394c,metalness:.1,roughness:.16,transmission:.06,transparent:true,opacity:.86});
  const red=new THREE.MeshStandardMaterial({color:0xc10e1d,emissive:0x290005});
  const chrome=new THREE.MeshStandardMaterial({color:0xc9ced2,metalness:.75,roughness:.25});

  const L=data.length, W=data.width, H=data.height;
  const body=new THREE.Mesh(new THREE.BoxGeometry(W, .52, L*.72), bodyMat); body.position.y=.52;body.castShadow=true;
  body.geometry.translate(0,0,.05);
  group.add(body);

  let cabinLen=L*.38,cabinZ=-.03,cabinH=.56;
  if(data.type==='hatch'){cabinLen=L*.43;cabinZ=-.02;cabinH=.62}
  if(data.type==='suv'){cabinLen=L*.47;cabinH=.68;cabinZ=-.02}
  if(data.type==='suvCoupe'){cabinLen=L*.45;cabinH=.63;cabinZ=.05}
  if(data.type==='typeR'){cabinLen=L*.39;cabinH=.53;cabinZ=.02}
  const cabin=new THREE.Mesh(new THREE.BoxGeometry(W*.82,cabinH,cabinLen),glass);cabin.position.set(0,.94,cabinZ);cabin.castShadow=true;group.add(cabin);

  // roof / silhouette cap
  const roof=new THREE.Mesh(new THREE.BoxGeometry(W*.68,.12,cabinLen*.72),bodyMat);roof.position.set(0,1.23,cabinZ-.02);group.add(roof);

  // hood + bumpers
  const hood=new THREE.Mesh(new THREE.BoxGeometry(W*.94,.16,L*.22),bodyMat);hood.position.set(0,.78,L*.31);group.add(hood);
  const grille=new THREE.Mesh(new THREE.BoxGeometry(W*.60,.18,.06),dark);grille.position.set(0,.54,L*.365+.035);group.add(grille);
  const chromeBar=new THREE.Mesh(new THREE.BoxGeometry(W*.48,.035,.065),chrome);chromeBar.position.set(0,.61,L*.366+.04);group.add(chromeBar);

  const headGeo=new THREE.BoxGeometry(.31,.16,.07), headMat=new THREE.MeshStandardMaterial({color:0xeef7ff,emissive:0x8295a5,emissiveIntensity:.6});
  for(const x of [-W*.32,W*.32]){const h=new THREE.Mesh(headGeo,headMat);h.position.set(x,.68,L*.367+.05);group.add(h)}
  const tailGeo=new THREE.BoxGeometry(.32,.16,.07);
  for(const x of [-W*.32,W*.32]){const t=new THREE.Mesh(tailGeo,red);t.position.set(x,.67,-L*.365-.04);group.add(t)}

  // wheels
  const wheelGeo=new THREE.CylinderGeometry(.31,.31,.20,18);wheelGeo.rotateZ(Math.PI/2);
  const wheelPositions=[[-W*.50,.35,L*.24],[W*.50,.35,L*.24],[-W*.50,.35,-L*.24],[W*.50,.35,-L*.24]];
  wheelPositions.forEach(([x,y,z])=>{const wh=new THREE.Mesh(wheelGeo,dark);wh.position.set(x,y,z);wh.castShadow=true;group.add(wh)});

  // Type R cues
  if(data.type==='typeR'){
    const wing=new THREE.Mesh(new THREE.BoxGeometry(W*.76,.08,.18),dark);wing.position.set(0,1.08,-L*.42);group.add(wing);
    const posts=new THREE.BoxGeometry(.05,.28,.05);for(const x of [-W*.27,W*.27]){const p=new THREE.Mesh(posts,dark);p.position.set(x,.94,-L*.39);group.add(p)}
    const lip=new THREE.Mesh(new THREE.BoxGeometry(W*.92,.08,.16),red);lip.position.set(0,.29,L*.39);group.add(lip);
  }
  if(data.type==='suv'||data.type==='suvCoupe'){
    const cladding=new THREE.Mesh(new THREE.BoxGeometry(W*1.01,.16,L*.7),dark);cladding.position.y=.31;group.add(cladding);
  }

  group.userData.radius=Math.max(W*.55,L*.28);
  return group;
}
